...
async fn prepare_payload_generic(
    ...
    let mut shufflings = Shufflings::for_state(&advanced_empty_state, &spec).unwrap();
    complete_state_advance(
        &mut advanced_empty_state,
        &mut shufflings,
        None,
        prepare_slot,
        None,
...
async fn gloas_block_production_caches_blobs_for_column_publishing() {
    ...
    complete_state_advance(&mut state, &mut shufflings, None, slot, None, &spec).unwrap();
