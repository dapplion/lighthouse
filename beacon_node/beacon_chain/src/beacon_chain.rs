...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    fn is_block_data_imported(&self, block_root: Hash256, slot: Slot) -> Result<bool, BlockError> {
        let is_gloas = self
            .spec
            .fork_name_at_slot::<T::EthSpec>(slot)
            .gloas_enabled();
        ...
        let fork_choice = self.canonical_head.fork_choice_read_lock();
        let block_known = fork_choice.contains_block(&block_root);
        if !block_known {
            return Ok(false);
        }
        ...
        Ok(fork_choice.is_payload_received(&block_root))
    }
