...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    pub fn validator_attestation_duties(
        ...
        // The verdict gates whether the VC may sign, so it has to describe the execution the
        // head's branch actually ran. An empty head runs an ancestor's payload, not its own.
    ...
    fn filter_optimistic_attestation(
        ...
        let fork_choice = self.canonical_head.fork_choice_read_lock();
        // The aggregator signs over this attestation, so the subject is the node the attestation
        // supports. A same-slot vote supports `PENDING`, not the block's own payload.
    ...
    pub fn produce_unaggregated_attestation(
        ...
        // Spec: an optimistic validator MUST NOT participate in attestation, where optimistic
        // means `is_optimistic(head)`. The subject is the head node, not the block being attested
        // to: `beacon_block_root` is either the head itself or one of its ancestors, whose
        // payloads the head's branch already ran.
        match self
            .canonical_head
            .fork_choice_read_lock()
            .get_node_execution_status(head_node)?
