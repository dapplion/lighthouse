...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    fn import_block_update_metrics_and_events(
        ...
        if block.slot() + 2 * T::EthSpec::slots_per_epoch() >= current_slot {
            ...
            if let Ok(proof) = block.body().whisk_opening_proof() {
                if proof == &WhiskTrackerProof::<T::EthSpec>::default() {
                    metrics::inc_counter(&metrics::BLOCK_WHISK_OPENING_PROOF_ZERO);
                }
            }
            if let Ok(proof) = block.body().whisk_registration_proof() {
                if proof == &WhiskTrackerProof::<T::EthSpec>::default() {
                    metrics::inc_counter(&metrics::BLOCK_WHISK_REGISTRATION_PROOF_ZERO);
                }
            }
            if let Ok(proof) = block.body().whisk_shuffle_proof() {
                // TODO WHISK: prevent having to allocate an empty proof
                if proof == &WhiskShuffleProof::<T::EthSpec>::default() {
                    metrics::inc_counter(&metrics::BLOCK_WHISK_SHUFFLE_PROOF_ZERO);
                }
            }
        }
            ...
            BeaconState::Capella(BeaconStateCapella {
                ...
                let (whisk_post_shuffle_trackers, whisk_shuffle_proof) = if should_shuffle_trackers(
                    ...
                    let pre_shuffle_trackers: Vec<WhiskTracker> = shuffle_indices
                        ...
                        .collect();
                    drop(shuffle_indices_timer);

                    let _shuffle_proof_timer =
                        metrics::start_timer(&metrics::BLOCK_PRODUCTION_SHUFFLE_PROOF_TIMES);
