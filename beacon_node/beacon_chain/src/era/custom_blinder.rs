...
use types::{EthSpec, Slot, Withdrawals, transactions_tree_hash_root_from_ssz_bytes};
...
pub fn custom_blind_capella<E: EthSpec>(signed: &[u8]) -> Result<Vec<u8>, ssz::DecodeError> {
    ...
    let transactions_root = transactions_tree_hash_root_from_ssz_bytes::<E>(txs)
        .map_err(|e| ssz::DecodeError::BytesInvalid(format!("transactions hash: {e:?}")))?;
