...
fn sync_info_with_head_root(head_root: Hash256) -> SyncInfo {
    SyncInfo {
...
fn finalized_sync_not_enough_custody_peers_on_start(config: Config) {
    ...
    // We are a supernode, and just added a single non-supernode peer. The custody by root request
    // will stall as many columns have zero peers.
