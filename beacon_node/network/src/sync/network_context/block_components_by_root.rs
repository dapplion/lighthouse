...
use beacon_chain::{BeaconChainTypes, get_block_root};
use lighthouse_network::PeerId;
use lighthouse_network::service::api_types::{
    BlobsByRootRequestId, BlocksByRootRequestId, ComponentsByRootRequestId, CustodyByRootRequestId,
};
use parking_lot::RwLock;
use std::collections::{HashMap, HashSet};
use std::sync::Arc;
use types::{
    BlobSidecar, ChainSpec, DataColumnSidecarList, EthSpec, Hash256, RuntimeVariableList,
    SignedBeaconBlock,
};
...
enum State<E: EthSpec> {
    BlocksRequest {
        blocks_request: DownloadRequest<BlocksByRootRequestId, Arc<SignedBeaconBlock<E>>>,
    },
    DataRequest {
...
enum DataRequest<E: EthSpec> {
    Deneb {
        blobs_request: DownloadRequest<BlobsByRootRequestId, Vec<Arc<BlobSidecar<E>>>>,
    },
    Fulu {
        custody_request:
            DownloadRequest<CustodyByRootRequestId, DataColumnSidecarList<E>, PeerGroup>,
    },
}
...
impl From<DownloadRequestError> for Error {
    fn from(e: DownloadRequestError) -> Self {
        match e {
            ...
            DownloadRequestError::TooManyErrors(e) => todo!(),
        }
    }
}
...
impl<T: BeaconChainTypes> BlockComponentsByRootRequest<T> {
    pub fn new(
        ...
        let blocks_req_id = cx.send_blocks_by_root_request(id, peers.clone(), block_root)?;
                    ...
                    if block_has_data && fork.fulu_enabled() {
                        let req_id = cx
                            ...
                            .map_err(|e| match e {
                                ...
                                RpcRequestSendError::NoPeer(_) => Error::InternalError(
                        ...
                        let blobs_request = DownloadRequest::new();
                        blobs_request.on_download_start(req_id)?;
                        self.state = State::DataRequest {
                            block: block.clone(),
                            block_peer: *block_peer,
            ...
            State::DataRequest {
                ...
                DataRequest::Fulu {
                    ...
                    if let Some((columns, column_peers)) = custody_by_range_request.to_finished() {
                        ...
                        let rpc_block = couple_block_fulu(block.clone(), columns.to_vec())?;
                        Ok(Some((rpc_block, peer_group)))
                    } else {
    ...
    pub fn on_blocks_by_root_result(
        ...
        data: Result<Arc<SignedBeaconBlock<T::EthSpec>>, RpcResponseError>,
        peer_id: PeerId,
        cx: &mut SyncNetworkContext<T>,
    ) -> BlockComponentsByRootRequestResult<T::EthSpec> {
        match &mut self.state {
            State::BlocksRequest { blocks_request } => {
                blocks_request.on_result(id, data, peer_id)?;
            }
            _ => {
    ...
    pub fn on_blobs_by_root_result(
        ...
        data: Result<Vec<Arc<BlobSidecar<T::EthSpec>>>, RpcResponseError>,
        peer_id: PeerId,
        cx: &mut SyncNetworkContext<T>,
    ) -> BlockComponentsByRootRequestResult<T::EthSpec> {
        match &mut self.state {
            State::DataRequest {
                ...
                blobs_request.on_result(id, peer_id, data)?;
            }
            _ => {
                return Err(Error::InternalError(
    ...
    pub fn on_custody_by_root_result(
        ...
        data: Result<DataColumnSidecarList<T::EthSpec>, RpcResponseError>,
        peers: PeerGroup,
        cx: &mut SyncNetworkContext<T>,
    ) -> BlockComponentsByRootRequestResult<T::EthSpec> {
        match &mut self.state {
...
fn couple_block_fulu<E: EthSpec>(
    ...
    RpcBlock::new_with_custody_columns(Some(block_root), block, data_columns_with_block_root)
