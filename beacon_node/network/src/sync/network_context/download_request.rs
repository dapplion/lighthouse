...
impl<I: std::fmt::Display + PartialEq, T> DownloadRequest<I, T> {
    ...
    pub fn on_result(
        ...
        req_id: I,
        peer_id: PeerId,
        result: Result<T, RpcResponseError>,
    ) -> Result<(), Error> {
        match &self.status {
            Status::Downloading(expected_req_id) => {
                ...
                match result {
                    Ok(data) => {
                        self.status = Status::Downloaded(peer_id, data);
                    }
                    Err(e) => {
            ...
            other => Err(Error::InternalError(format!(
                "bad state on_result expected Downloading got {}",
                Into::<&'static str>::into(other),
            ))),
        }
    }
    ...
    pub fn is_complete(&self) -> Option<&T> {
        match &self.status {
            Status::Downloaded(_, data) => Some(data),
            _ => None,
        }
    }

    pub fn complete(self) -> Result<(PeerId, T), Error> {
        match self.status {
            Status::Downloaded(peer_id, data) => Ok((peer_id, data)),
            other => Err(Error::InternalError(format!(
