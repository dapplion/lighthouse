...
use crate::sync::network_context::{BatchPeers, RpcResponseError, SyncNetworkContext};
    ...
    T: BeaconChainTypes,
{
    ...
    pub fn blocks_by_range_response(
        ...
        match self.chains.call_by_id(chain_id, |chain| {
            chain.on_block_response(network, batch_id, batch_peers.clone(), request_id, blocks)
        }) {
    ...
    pub fn inject_error(
        ...
        match self.chains.call_by_id(chain_id, |chain| {
            chain.inject_error(network, batch_id, batch_peers, request_id, err)
        }) {
            Ok((removed_chain, sync_type)) => {
                if let Some((removed_chain, remove_reason)) = removed_chain {
                    self.on_chain_removed(
