...
/// SIMD hashing of 64-byte chunks via the `hashtree` library.
///
/// `hashtree` only hashes whole 64-byte blocks (the merkle parent-node case), so it
/// complements rather than replaces the `sha2`/`ring` backends.
#[cfg(all(
    feature = "hashtree",
...
mod hashtree_impl {
    ...
    #[inline]
    pub fn hash64(input: &[u8]) -> Option<[u8; super::HASH_LEN]> {
        debug_assert_eq!(input.len(), 64);
        if !*AVAILABLE {
            return None;
        }
        let mut output = [0; super::HASH_LEN];
        hashtree_rs::hash(&mut output, input, 1);
        Some(output)
    }
}
...
mod hashtree_impl {
    #[inline]
    pub fn hash64(_input: &[u8]) -> Option<[u8; super::HASH_LEN]> {
        None
    }
}
...
pub fn hash(input: &[u8]) -> Vec<u8> {
    if input.len() == 64 {
        if let Some(output) = hashtree_impl::hash64(input) {
...
pub fn hash32_concat(h1: &[u8], h2: &[u8]) -> [u8; 32] {
    if h1.len() == 32 && h2.len() == 32 {
        let mut block = [0; 64];
        block[..32].copy_from_slice(h1);
        block[32..].copy_from_slice(h2);
        return hash_fixed(&block);
    }
...
mod tests {
    ...
    /// The hashtree fast path must agree with the streaming implementations on
    /// 64-byte inputs.
    #[test]
    fn hashtree_matches_streaming_backend() {
        let block: Vec<u8> = (0u8..64).collect();

        let mut ctxt = DynamicContext::new();
        ctxt.update(&block);
        let expected = ctxt.finalize();

        assert_eq!(hash_fixed(&block), expected);
        assert_eq!(hash(&block), expected.to_vec());
        assert_eq!(hash32_concat(&block[..32], &block[32..]), expected);
    }
}
