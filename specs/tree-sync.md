...
owners(R) = the chains awaiting the call for `R` — those whose `roots` ⊆ `R`, ascending by min slot
...
Merge(Y, Z) = c           Backfill only
  guard Z.state = Anchored(Y.roots[0]) ∧ Y.peers = Z.peers ∧ Y.roots ≠ []
  c = { roots: Z.roots ++ Y.roots, peers: Y.peers, state: Y.state, errors: Y.errors }
```

`Y ⊎ Z = c`. `Merge(Split(c, r)) = c` for `Backfill`. Callers own `loc`. No transition invokes `Merge`; it is a background compaction. Both halves keep awaiting the shared in-flight call, and `OnDownload`/`OnProcess` deliver to both — `owners(R)`, not the half that issued it.
...
- `loc(r) = c`, `c.state = Discovering(r)` → `c.roots = []` → `c.peers ∪= P`; else skip — `c.roots` are `r`'s descendants
- `loc(r) = c` → `(Y,Z) := Split(c,r)`; `Y.peers ∪= P`; `loc[Z.roots] := Z`

then ascend: for each ancestor root `p` by `parent` links — `loc(p) = c'` ∧ `p ∈ c'.roots` → `Split(c', p)`; add `P` to the older half; continue from its `parent`. `p ∉ c'.roots` → stop.
...
- `Err` ∨ `Ok([])` → `errors += 1`; `errors > RETRY_MAX` → `Drop(c)`, else `SendHeaders(c, next)`
- `Ok(headers)`, `headers ≠ []` → `errors := 0`; for `header ∈ headers`:
...
The ascent on attaching matters because `Search`'s ascent cannot reach ancestors that did not exist yet. A peer that claimed this chain's tip holds them too, and without this the parent chain can be left holding only failing peers. It splits at each `p` because `p` may be interior to the chain owning it, whose newer roots are on another branch and were never claimed (Inv 3).
...
- `Ok(blocks)` not covering `R` → `report_peer`; as `Err`
...
**`Drop(x)`** — remove `x`, every `c` whose `parent` is a root `x` claimed, and every `loc` entry pointing at `x`; transitively. A chain claims `x.roots` and its `Discovering(next)` root.
...
2. `c.roots` contiguous by `parent_root`, tip first, distinct.
...
5. `Ready(blocks) ∨ Processing(blocks)` ⟹ `blocks` is `c.roots` reversed — oldest first, one block per root — with `∀i>0. parent(blocks[i]) = blocks[i−1]`; `Processing` also ⟹ `parent(blocks[0]) ∈ FC ∨ ∈ dom(loc)` — `SendProcess`'s guard establishes the stronger `∈ FC` for the whole segment, which `Split` does not preserve. `roots` is tip first and `blocks` is import order, so the reversal is where they meet.
6. `state = Anchored(p)` ⟹ `p ∈ FC ∨ p ∈ dom(loc)`.

## Liveness

- Discovery terminates: `slot` strictly decreases (Inv 4), bounded below by `slot(finalized)`.
- With ≥1 honest peer holding the chain in `c.peers`, a chain is dropped only after `RETRY_MAX` consecutive failures of one step; every other root reaches `FC`.
