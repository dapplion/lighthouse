...
pub trait Handler {
    ...
    /// Hook to skip individual test cases before their files are loaded.
    fn should_run_case(&self, _case_path: &Path) -> bool {
        true
    }
    ...
    fn run_for_fork(&self, fork_name: ForkName) {
        ...
        let test_cases = fs::read_dir(&handler_path)
            ...
            .filter_map(as_directory)
            .filter(|test_case_dir| self.should_run_case(&test_case_dir.path()))
            .map(|test_case_dir| {
...
/// Runner for the fork choice compliance test suites.
///
...
/// them yet. Cases listed in `fork_choice_compliance_excluded.txt` are skipped entirely; the same
/// list exempts their files from `check_all_files_accessed.py`. Fixes should delete entries from
/// that list so that the newly-passing cases become mandatory.
...
impl<E: EthSpec> ForkChoiceComplianceHandler<E> {
    pub fn new(handler_name: &str) -> Self {
        Self {
            handler_name: handler_name.into(),
            excluded_prefixes: compliance_exclusions(),
            _phantom: PhantomData,
        }
    }
}

/// Load the known-failing case list shared with `check_all_files_accessed.py`.
///
/// Each non-comment line is a path prefix relative to `consensus-spec-tests/tests`.
fn compliance_exclusions() -> Vec<String> {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fork_choice_compliance_excluded.txt");
...
impl<E: EthSpec + TypeName> Handler for ForkChoiceComplianceHandler<E> {
    ...
    fn is_enabled_for_fork(&self, fork_name: ForkName) -> bool {
        // The vectors are generated with unverifiable signatures (`bls_setting: 2`) and only
        // exist for Altair onwards.
        cfg!(feature = "fake_crypto") && fork_name.altair_enabled()
    }

    fn should_run_case(&self, case_path: &Path) -> bool {
        let tests_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("consensus-spec-tests")
            .join("tests");
        let Ok(rel_path) = case_path.strip_prefix(&tests_dir) else {
            return true;
        };
        let rel_path = rel_path.to_string_lossy();
        // Match whole path components only: an excluded `case_1` must not skip `case_19`.
