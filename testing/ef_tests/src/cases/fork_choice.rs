...
use state_processing::{GloasVerificationContext, VerifySignatures, per_slot_processing};
...
impl<E: EthSpec> Tester<E> {
    ...
    /// Stand in for the state advance timer, which does not run in these tests. FCR refuses to
    /// advance a state under the fork choice lock, so without a cached head state in the current
    /// epoch it would skip confirmation and the FCR vectors could never pass.
