...
impl FastConfirmationRule {
    ...
    /// when no head state is available and confirmation itself is skipped: a missed rotation
    /// leaves `current_epoch_observed_justified` stale for a whole epoch, which blocks the
    /// restart branch of `get_latest_confirmed`.
