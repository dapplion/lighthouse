...
use alloc::vec::Vec;
pub use primitives::{
    ArithError, Checkpoint, Epoch, EthSpec, Hash256, SafeArith, Slot, SlotAssignments,
    SlotAssignmentsError, SlotsPerEpoch, VoteTracker, Votes,
};
pub use store::ForkChoiceStore;
use store::iter_block_roots;
...
pub enum Error {
    ...
    SlotAssignmentsError(SlotAssignmentsError),
...
/// Which branches of `get_latest_confirmed` fired. The host logs and counts
/// these; the rule only reports them.
...
#[derive(Debug)]
pub struct FastConfirmationRule<A: SlotAssignments> {
    // === Output ===
...
impl<A: SlotAssignments> FastConfirmationRule<A> {
    ...
    pub fn new(
        ...
        checkpoint_balance: BalanceSourceData,
        ...
        // checkpoint's epoch.
        if checkpoint_balance.epoch != finalized_checkpoint.epoch {
    ...
    pub fn head_balance_source(&self) -> &BalanceSourceData {
    ...
    pub fn on_fast_confirmation<E: EthSpec, V: Votes + ?Sized>(
        &mut self,
        head_root: Hash256,
        finalized_checkpoint: &Checkpoint,
        ...
        head_balance_source: Option<BalanceSourceData>,
        slot_assignments: A,
        checkpoint_balance: Option<BalanceSourceData>,
        ...
        if !self.spec_test_mode {
            (self.confirmed_root, outcome) = self.get_latest_confirmed::<E, _>(
                head_root,
                finalized_checkpoint,
                unrealized_justified_checkpoint,
                current_slot,
    ...
    pub fn update_fast_confirmation_variables<E: EthSpec>(
        ...
        if let Some(head_balance_source) = head_balance_source {
            ...
            if self.will_rotate::<E>(current_slot) {
                ...
                let new_current =
                    if new_current_cp == self.current_epoch_observed_justified.checkpoint() {
                        ...
                        let checkpoint_balance = checkpoint_balance
                            .ok_or(Error::MissingCheckpointState(new_current_cp))?;
                        // Sanity: the supplied state must be the checkpoint's state, advanced to the
                        // checkpoint's epoch.
                        if checkpoint_balance.epoch != new_current_cp.epoch {
                            return Err(Error::MissingCheckpointState(new_current_cp));
                        }
                        CheckpointAndBalance::new(new_current_cp, checkpoint_balance)
                    };
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn get_latest_confirmed<E: EthSpec, V: Votes + ?Sized>(
        &self,
        head_root: Hash256,
        finalized_checkpoint: &Checkpoint,
        unrealized_justified_checkpoint: &Checkpoint,
        ...
        let should_revert_to_finalized_reason = if confirmed_block_epoch_result
            ...
            && let Some(chain_unsafe_reason) = self.is_confirmed_chain_safe::<E, _>(
        ...
        if get_block_epoch::<E>(confirmed_root, proto_array)?.safe_add(1)? >= current_epoch {
            let advanced = self.find_latest_confirmed_descendant::<E, _>(
                confirmed_root,
                head_root,
                unrealized_justified_checkpoint,
                current_slot,
    ...
    #[allow(clippy::too_many_arguments)]
    fn find_latest_confirmed_descendant<E: EthSpec, V: Votes + ?Sized>(
        &self,
        ...
        if get_block_epoch::<E>(confirmed_root, proto_array)?.safe_add(1)? == current_epoch
            && get_voting_source_epoch::<E>(self.previous_slot_head, current_slot, proto_array)?
                ...
                >= current_epoch
            && (is_start_slot_at_epoch::<E>(current_slot)
                || (self.will_no_conflicting_checkpoint_be_justified::<E, _>(
                    head_root,
                    unrealized_justified_checkpoint,
                    current_slot,
                    proto_array,
                ...
                >= current_epoch
        {
            ...
            for block_root in &canonical_roots {
                ...
                if block_epoch > tentative_epoch
                    && !self.will_current_target_be_justified::<E, _>(
                        head_root,
                        current_slot,
                        proto_array,
                        votes,
    ...
    fn is_confirmed_chain_safe<E: EthSpec, V: Votes + ?Sized>(
        &self,
        ...
        for root in &chain_roots {
            if let Confirmation::NotConfirmed(unconfirmed) = self.is_one_confirmed::<E, _>(
                self.get_previous_balance_source(),
                *root,
                &attestation_scores,
                current_slot,
    ...
    fn get_block_support_between_slots<V: Votes + ?Sized>(
        ...
        for (val_idx, balance) in balance_source.unslashed_and_active_indices() {
            if balance > 0
                ...
                && votes.root(val_idx) == Some(&block_root.0)
            {
    ...
    fn compute_empty_slot_support_discount<E: EthSpec, V: Votes + ?Sized>(
        &self,
        balance_source: &BalanceSourceData,
        block_root: Hash256,
        proto_array: &dyn ForkChoiceStore,
        ...
        let parent_block = parent_root(block_root, proto_array)?;
        let parent_slot = get_block_slot(parent_block, proto_array)?;
    ...
    fn get_support_discount<E: EthSpec, V: Votes + ?Sized>(
        &self,
        balance_source: &BalanceSourceData,
        block_root: Hash256,
        proto_array: &dyn ForkChoiceStore,
        ...
        self.compute_empty_slot_support_discount::<E, _>(
            balance_source,
            block_root,
            proto_array,
            votes,
    ...
    fn compute_safety_threshold<E: EthSpec, V: Votes + ?Sized>(
        &self,
        balance_source: &BalanceSourceData,
        block_root: Hash256,
        current_slot: Slot,
        ...
        let support_discount = self.get_support_discount::<E, _>(
            balance_source,
            block_root,
            proto_array,
            votes,
    ...
    #[allow(clippy::too_many_arguments)]
    fn will_no_conflicting_checkpoint_be_justified<E: EthSpec, V: Votes + ?Sized>(
        &self,
        head_root: Hash256,
        unrealized_justified_checkpoint: &Checkpoint,
        current_slot: Slot,
        ...
        let honest_ffg = honest_ffg_support.get_or_compute(|| {
            self.compute_honest_ffg_support::<E, _>(
                head_root,
                current_slot,
                proto_array,
                votes,
    ...
    fn will_current_target_be_justified<E: EthSpec, V: Votes + ?Sized>(
        &self,
        head_root: Hash256,
        current_slot: Slot,
        proto_array: &dyn ForkChoiceStore,
    ...
    pub fn get_current_target_score<E: EthSpec, V: Votes + ?Sized>(
        &self,
        head_root: Hash256,
        current_slot: Slot,
        proto_array: &dyn ForkChoiceStore,
    ...
    fn compute_honest_ffg_support<E: EthSpec, V: Votes + ?Sized>(
        &self,
        head_root: Hash256,
        current_slot: Slot,
        proto_array: &dyn ForkChoiceStore,
        ...
        let ffg_support_for_checkpoint = self.get_current_target_score::<E, _>(
            head_root,
            current_slot,
            proto_array,
            votes,
    ...
    #[allow(clippy::too_many_arguments)]
    fn is_one_confirmed<E: EthSpec, V: Votes + ?Sized>(
        &self,
        balance_source: &BalanceSourceData,
        block_root: Hash256,
        ...
        let safety_threshold = self.compute_safety_threshold::<E, _>(
            balance_source,
            block_root,
            current_slot,
            proto_array,
...
/// Spec: `get_block_slot`.
fn get_block_slot(root: Hash256, proto_array: &dyn ForkChoiceStore) -> Result<Slot, Error> {
    proto_array
        .block_slot(root)
...
fn parent_root(root: Hash256, proto_array: &dyn ForkChoiceStore) -> Result<Hash256, Error> {
    let (_, parent) = proto_array
        .slot_and_parent(root)
        .ok_or(Error::NodeNotFound(root))?;
    parent.ok_or(Error::ParentRootNotFound(root))
}
...
fn is_optimistic_or_invalid(
    root: Hash256,
    proto_array: &dyn ForkChoiceStore,
) -> Result<bool, Error> {
    Ok(proto_array.is_optimistic_or_invalid(root).unwrap_or(false))
}
...
fn is_ancestor(
    ...
    Ok(iter_block_roots(proto_array, block_root)
...
fn get_ancestor(
    ...
    iter_block_roots(proto_array, block_root)
        .find(|(_, s)| *s <= slot)
        .map(|(r, _)| r)
...
fn get_ancestor_roots(
    ...
    for (root, slot) in iter_block_roots(proto_array, block_root) {
...
fn unrealized_justification_of(
    ...
    proto_array
        .unrealized_justified_checkpoint(root)
...
fn get_voting_source_epoch<E: EthSpec>(
    ...
    let block_slot = get_block_slot(root, proto_array)?;
    let current_epoch = current_slot.epoch(E::slots_per_epoch());
